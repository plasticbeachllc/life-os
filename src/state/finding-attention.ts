...
import type {
  AttentionSignal, ValidatedCommunicationContext, ValidatedFindingRelation,
...
export const findingAttentionBuilder: ProjectionBuilder<FindingAttentionInput, FindingAttentionState> = {
  ...
  inputs: ({ now, active, tasks, communicationContexts, relations }) => [
    ...
    ...communicationContexts.map((context) => ({
      type: "finding_communication_context", id: context.finding_id, hash: context.content_hash,
    })),
    ...relations.map((relation) => ({
      type: "finding_relation", id: relation.relation_id, hash: relation.content_hash,
    })),
  ],
  build: ({ now, active, tasks, communicationContexts, relations }) => {
    ...
    const resolution = resolveAttention({
      activeFindings: active, tasks, now, communicationContexts, relations,
    });
...
export function rebuildFindingAttentionState(input: {
  ...
  const communicationContexts = findingStore.activeCommunicationContexts();
  const relations = findingStore.activeRelations();
  return materializeProjection({
    store: input.store, builder: findingAttentionBuilder,
    value: { now, active, tasks, communicationContexts, relations }, now,
  }).state;
}
