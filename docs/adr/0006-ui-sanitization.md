...
mutation deny list. Feedback is append-only and requires exact same-origin JSON plus a short-lived random
token bound to the HttpOnly browser session and the precise opaque subject IDs/kinds issued in that workspace
snapshot. It cannot authorize or apply an effect.
